...
export async function getTrandingNews() {
    const json = await fetchJson("/api/v1/frontend/news/trending?limit=20");
    return (json?.data || []).map(normalizeNewsItem);
}

export async function submitContactForm(data) {
    try {
        const res = await fetch(`${BASE_URL}/api/v1/communication/contact`, {
            method: "POST",
            headers: {
...
export async function subscribeNewsletter(email) {
    try {
        const res = await fetch(`${BASE_URL}/api/v1/communication/newsletter/subscribe`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
...
export async function getRelatedNews(slug) {
    const json = await fetchJson(`/api/v1/frontend/news/${encodeURIComponent(slug)}/related`);
    return (json?.data || []).map(normalizeNewsItem);
}
...
export async function getTrendingTags() {
    const json = await fetchJson("/api/v1/frontend/categories/trending?limit=10");
