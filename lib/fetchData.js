...
/** Old shared URLs used /news/[numericId]; slugs are non-numeric. */
const LEGACY_NEWS_NUMERIC_ID = /^\d+$/;

function pickResolvedSlugFromJson(json) {
    ...
    if (data && typeof data === "object" && typeof data.slug === "string" && data.slug.trim()) {
...
/**
 * Resolves a legacy numeric news id to the current slug via the Render API.
 * @param {string} id
 * @returns {Promise<string|null>}
 */
export async function resolveLegacyNewsSlugFromNumericId(id) {
    const trimmed = String(id ?? "").trim();
    if (!LEGACY_NEWS_NUMERIC_ID.test(trimmed)) {
        return null;
    }
    try {
        const url = `${BASE_URL}/api/news/resolve-slug/${encodeURIComponent(trimmed)}`;
        const res = await fetch(url, { cache: "no-store" });
        if (!res.ok) {
            return null;
        }
        const json = await res.json();
        return pickResolvedSlugFromJson(json);
    } catch {
        return null;
    }
}
...
/**
 * Dedupes legacy resolution between generateMetadata and the page in one request.
 * @param {string} segment — dynamic [slug] param (may be a legacy numeric id)
 */
export const getLegacyNewsSegmentResolution = cache(async (segment) => {
    const trimmed = String(segment ?? "").trim();
    if (!LEGACY_NEWS_NUMERIC_ID.test(trimmed)) {
        return { isLegacyNumeric: false, resolvedSlug: null };
    }
    const resolvedSlug = await resolveLegacyNewsSlugFromNumericId(trimmed);
    return { isLegacyNumeric: true, resolvedSlug };
});
